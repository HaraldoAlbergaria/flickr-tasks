...
def addPhotoToGroup(photo_id, photo_title, photo_favs, is_public, not_in_group):
    if photo_favs >= 1 and is_public == 1 and not_in_group and not hasTag(photo_id, 'DNA'):
        try:
...
def remPhotoFromGroup(photo_id, photo_title, photo_favs):
    if photo_favs == 0 or hasTag(photo_id, 'DNA'):
        try:
...
def processPhoto(photo_id, photo_title, user_id):
    ...
    addPhotoToGroup(photo_id, photo_title, photo_favs, is_public, not_in_group)
    remPhotoFromGroup(photo_id, photo_title, photo_favs)
    print(' ')
