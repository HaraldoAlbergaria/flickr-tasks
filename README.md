...
- **generate_map**: Inside directory 'generate_map' create a link (or copy) to the file __api_credentials.py__. There are three additional files necessary to run the scripts:
    - **procs.py**: There is no need to edit this file, keep it as is.
    - **header.html**: This file contains the initial code for the generated _html_ file. An API access token must be inserted in the proper field in order to the map can be loaded. To get one, [sign up](https://account.mapbox.com/auth/signup/) to [Mapbox](https://www.mapbox.com/). The generated file will be **map.html**.
